...
def fetch_latest_prices():
	...
	cursor = conn.cursor()
	query = """
	SELECT product_id, pharmacy_name, price_value, expiration_date, fetched_at
	FROM pharmacy_prices
	WHERE (product_id, price_value, expiration_date, fetched_at) IN (
		SELECT product_id, price_value, expiration_date, MAX(fetched_at)
		FROM pharmacy_prices
		GROUP BY product_id, pharmacy_name, expiration_date
	)
	"""
